...
export class SmokeComponent implements AfterViewInit
{
  ...
  private init()
  {
    ...
    const boxHeight = 200;
    const boxDepth = 200;
    const materialColour0 = 0x000000;
    this.geometry = new THREE.BoxGeometry(boxWidth, boxHeight, boxDepth);
    this.material = new THREE.MeshLambertMaterial({ color: materialColour0, wireframe: false });
    this.mesh = new THREE.Mesh(this.geometry, this.material);

    const lightColour = this.lightColours[this.lightColourIndex];
    ...
    const textureURL = 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/95637/Smoke-Element.png';
    const materialColour1 = 0xffffff
    const materialTransparent = true;
    let smokeGeometry = new THREE.PlaneGeometry(planeWidth, planeHeight);
    let smokeTexture = new THREE.TextureLoader().load(textureURL);
    let smokeMaterial = new THREE.MeshLambertMaterial({color: materialColour1, map: smokeTexture, transparent: materialTransparent});
    this.smokeParticles = [];
